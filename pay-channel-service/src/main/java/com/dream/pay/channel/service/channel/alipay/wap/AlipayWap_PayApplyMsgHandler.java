 ...
 */
@Slf4j
public class AlipayWap_PayApplyMsgHandler extends FreemarkChannelMsgHandler<PayApplyReqDTO, PayApplyRepDTO> {
    @Override
    protected String getTemplatePath() {
        return AlipayConfigContants.WAP_PAY_TEMPLATE_PATH;
    }
    ...
    public PayApplyReqDTO beforBuildMsg(PayApplyReqDTO req, ChannelConfig channelConfig) throws ChannelMsgException {
        ...
        try {
            ...
            // 添加接口参数
            super.setParam("service", StringUtils.trim(config.getWapPayApplyService()));// 接口名称
            ...
            String callBackParams = req.getExt().get(ExtendParamKey.CALLBACK_PARAMS);
            if (StringUtils.isNoneBlank(callBackParams)) {
                super.setParam("return_url", StringUtils.trim(config.getPayReturnUrl() + callBackParams));// 页面跳转同步通知页面路径
            } else {
                super.setParam("return_url", StringUtils.trim(config.getPayReturnUrl()));// 页面跳转同步通知页面路径
            }
            super.setParam("show_url", StringUtils.trim(config.getShowUrl())); // 商品展示地址
            super.setParam("wap_pay_url", config.getWapPostUrl()); // 请求地址
            // 添加业务线等订单信息
