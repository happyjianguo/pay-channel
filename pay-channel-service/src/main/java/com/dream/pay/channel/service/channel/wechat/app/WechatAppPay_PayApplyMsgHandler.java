package com.dream.pay.channel.service.channel.wechat.app;
 ...
 */
@Slf4j
public class WechatAppPay_PayApplyMsgHandler extends XMLChannelMsgHandler<PayApplyReqDTO, PayApplyRepDTO> {
    ...
    public PayApplyReqDTO beforBuildMsg(PayApplyReqDTO req, ChannelConfig channelConfig) throws ChannelMsgException {
        ...
        try {
            ...
            super.setParam("attach", req.getPartnerId().getCode());// 业务线
            ...
            super.setParam("trade_type", "APP");// 统一下单接口trade_type:交易类型JSAPI--公众号支付、NATIVE--原生扫码支付、APP--app支付
            ...
            log.error("[微信APP]-[支付申请]-[报文拼装前设置参数]-[出现异常]", e);
    ...
    public PayApplyRepDTO resolveMsg(PayApplyReqDTO req, byte[] rtnMsg, ChannelConfig channelConfig)
        ...
        try {
            ...
            if (!resReturnCode.equals("SUCCESS")) {
                ...
                log.error("[微信APP]-[支付申请]-[解析返回报文]-[通信标示码不为成功[{}],返回信息为[{}]]", resReturnCode, resReturnMsg);
            ...
            if (!signFlag) {
                log.error("[微信APP]-[支付申请]-[解析返回报文]-[验签失败]-[{}]!=[{}]", resultMap.get("sign"), localSign);
            ...
            log.error("[微信APP]-[支付申请]-[解析返回报文]-[出现异常]", e);
            throw new ChannelMsgException(ChannelRtnCodeEnum.M10000, TradeStatus.FAIL);
        }
        return payApplyRepDTO;
    }
    ...
    private String buildRepContent(Wechatpay_ChannelConfig config, Map<String, Object> resultMap) {
        ...
        infoMap.put("partnerid", (String) resultMap.get("mch_id"));
        infoMap.put("prepayid", (String) resultMap.get("prepay_id"));
        infoMap.put("package", "Sign=WXPay");
